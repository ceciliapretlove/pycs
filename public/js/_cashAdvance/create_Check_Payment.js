$("#createCheckPaymentRequestForm").on('submit', function(e)
{
    $('#submitBtn').attr('disabled', 'disabled');
    $('.modal-message').html('');
    $('.error-message').html(""); //reset messages
    $('.form-group').removeClass('has-error');
    var post_data = $('#createCheckPaymentRequestForm').serialize();
    simPost(post_data, 'POST', '/cminaiwrzvq=vndgxhcaglo', createCheckPaymentRequest); 
    e.preventDefault();
    return false;
});


function createCheckPaymentRequest(x) {
	if(x == true){
